...
int rkmpp_get_##NAME##_format(rkformat *format, TYPE informat){ \
    for(int i=0; i < FF_ARRAY_ELEMS(rkformats); i++){ \
        if(rkformats[i].NAME == informat){ \
            ...
            format->multiplier = rkformats[i].multiplier;\
